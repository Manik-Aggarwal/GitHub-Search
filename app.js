...
// Displaying error if user not found
...
const getProfile = async (login) => {
  try {
      ...
      if (response.status !== 200) {
          if (response.status === 404) errorMessage(response.status);
  ...
  if (repos.length > 0) {
    repos.forEach((repo) => {
      ...
      if (repo.language !== null) {
        reposList += `<i class="fas fa-circle ${repo.language ? repo.language.toLowerCase() : ""}"></i> ${repo.language}`;
      }
  ...
  searchForm.addEventListener("submit", async (e) => {
    ...
    if (username.length > 0) {
      ...
      if (userProfile.login) {
        const userRepo = await getRepo(username);
